...
- 訪問者はログを読むだけ（読み取り専用）
- 管理者はログインして投稿・編集・削除・名前マスキングができる
...
| `ADMIN_PASSWORD` | 好きなパスワード（管理者ログインに使う） |
...
| ログインできる | ヘッダーの「管理」→ パスワードを入力してログイン |
| 投稿できる | ログイン後、「＋ 投稿する」から投稿 |
...
| ログイン | `/admin/login` | 管理者パスワードでログイン（30日間セッション維持） |
| 管理ページ | `/admin` | ログイン必須。投稿一覧・削除・エクスポート |
| 投稿ページ | `/posts/new` | ログイン必須 |
| 編集ページ | `/posts/[id]/edit` | ログイン必須 |
| マスキング設定 | `/settings` | ログイン必須。名前の置換ルールを設定 |
| ビューアー | `/viewer` | エクスポートしたMDファイルをブラウザで見る（認証不要） |
